...
- **`10.0.0.0/8` をデフォルトゲートウェイに設定しない**: `10.0.0.0/8` はインターネット到達性がない内部ネットワーク。デフォルトゲートウェイには `192.168.39.1` を使用する。OS セットアップ後に `ip route del default via 10.10.10.1` でゲートウェイを削除し、`ip route add default via 192.168.39.1` に切り替えること
- **SSH はプロジェクトローカルの `ssh/config` を使う**: SSH/SCP コマンドには必ず `-F ssh/config` を付けること。`~/.ssh/config` は使用しない。ホストエイリアス (`pve4`-`pve9`, `idrac7`-`idrac9`) が定義済み。ホスト鍵の削除は `ssh-keygen -R <ip> -f ssh/known_hosts` を使う。鍵が無い場合は `./ssh/setup.sh` で生成する
...
- `~/.ssh/`, `/proc/` 等のセンシティブパスへのアクセス (Read ツール使用を推奨。プロジェクト内の `ssh/` はセンシティブパスではない)
...
| `~/.ssh/`, `/proc/`, `/sys/` 等のセンシティブパス | sensitive path detection | `ls -la /home/ubuntu/.ssh/`, `cat /proc/net/udp` (プロジェクト内 `ssh/` は対象外) |
...
### よくあるルール違反（自動承認されないコマンド）

以下は過去のセッションで繰り返し発生した承認プロンプトの原因:

| NG パターン | 正しい方法 |
|------------|-----------|
| `find ... \| sort \| uniq` | Glob / Grep ツールを使う |
| `ls X; echo "---"; ls Y` | 個別 Bash 呼び出しに分割 |
| `python3 -c "...$()..."` | Write で tmp/<session-id>/ にファイル → `python3 tmp/<session-id>/script.py` |
...
- **リージョン構成**: `config/linstor.yml` の `regions` セクションで定義 (Region A: 4+5+6号機 IPoIB / Region B: 7+8+9号機 IPoIB)
- **主要スクリプト**:
