...
graph LR
    ...
    subgraph mgmt["管理ネットワーク (10.10.10.0/8)"]
        ...
        subgraph srv7["7号機"]
            iDRAC7["iDRAC7<br/>10.10.10.27"]
            PVE7["PVE<br/>10.10.10.207"]
        end
        subgraph srv8["8号機"]
            iDRAC8["iDRAC8<br/>10.10.10.28"]
            PVE8["PVE<br/>10.10.10.208"]
        end
        subgraph srv9["9号機"]
...
```
Region A (IPoIB)                          Region B (IPoIB)
┌─────────┐ ┌─────────┐ ┌─────────┐     ┌─────────┐ ┌─────────┐ ┌─────────┐
│ 4号機   │←→│ 5号機   │←→│ 6号機   │     │ 7号機   │←→│ 8号機   │←→│ 9号機   │
...
| ネットワーク | IPoIB | Ethernet (10GbE) |

**リージョン構成** (`config/linstor.yml`):

| リージョン | ノード | 接続 |
|-----------|--------|------|
| Region A | 4号機 + 5号機 + 6号機 | InfiniBand (IPoIB) |
| Region B | 7号機 + 8号機 + 9号機 | InfiniBand (IPoIB) |
...
7-9号機 (DELL R320) の iDRAC7 経由での操作。
