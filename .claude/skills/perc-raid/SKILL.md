...
**注意**: F2 メニュー初期カーソルは不定。ArrowDown x1 では Properties が開くことがある。
必ず ArrowUp x5 で先頭に移動してから ArrowDown x2 を使うこと。
...
6. **VNC は約 80 秒で接続が切れることがある**。`safe_key()` で自動再接続すること
...
**OK は PD リストから Tab x4** (Tab x5 は CANCEL)。
初期化確認ダイアログ: 初期フォーカスは Cancel → **Tab → Enter** で OK。

Tab 4 の OK ボタンは**オレンジ色にハイライト**されるので視覚的に確認可能。
